...
void vTaskFumaca(void *params)
{
    ...
    while(true)
    {
        adc_select_input(0); // Seleciona o canal ADC correspondente ao pino VRX_PIN
        uint16_t adc_value = adc_read(); // Lê o valor analógico do sensor MQ-135
        float ppm = (adc_value / 4095.0f) * LIMITE_FUMACA; // Converte o valor ADC para ppm (partes por milhão)
